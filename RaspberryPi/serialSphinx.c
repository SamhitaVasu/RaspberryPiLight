//Raspberry Pi voice activated light switch
//============================================================================
/*
  This program changes the state of an LED on an Arduino board when triggered
  by a word. Using pocketsphinx, what you say is turned into text.If the text
  is a valid command, it is sent to the Arduino using a serial library called
  <wiringSerial.h>. the LED (on the Arduino board) turns either on or off
  based on the command.
*/
/*
  To compile this program, the following must be installed:
     ~ CMU Pocketsphinx          ~ Sphinxbase
     ~ wiringPi                  ~ GNU GCC C compiler
*/
/*
  Type:
       gcc -Wall -o pocketsphinx_serial pocketSerial.c -lwiringPi -DMODELDIR=\"/usr/local/share/pocketsphinx/model\" -I/usr/local/include/sphinxbase -I/usr/local/include/pocketsphinx -L/usr/local/lib -lpocketsphinx -lsphinxbase -lsphinxad
  to compile.
*/
...
svopen(int i)
{
   if ((ard = serialOpen("/dev/ttyACM0", 9600)) < 0)
   {
...
/*
 * Continuous voice recognition from a file
 */
static void
recognize_from_file() {
   cont_ad_t *cont;
